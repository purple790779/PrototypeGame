...
## v0.4.0
- Feature: 길드 방문 이후 메인/서브 퀘스트 보드, 준비 단계, 보스전 확인 모달 추가
- Feature: 템플릿 조합 기반 서브 퀘스트 생성(희귀도/연계 퀘스트/보상/상점 방문 포함)
- Feature: 길드 상점 및 장비 슬롯/장착 스탯 반영 시스템 추가

## v0.3.10
- Travel: 길드까지 여정 단계 수를 4~7회로 확장하고, 전투/아이템/스탯업 이벤트 1회씩을 보장하는 플래그 로직 추가
- Travel: 최근 3개 이벤트 중복 방지와 아이템/스탯업 전용 이벤트를 포함한 신규 여정 데이터 추가
- UX: 이름 입력 후 스탯 빌드 5종(랜덤/근력/민첩/지력/밸런스) 선택 UI 및 15포인트 자동 분배 적용
